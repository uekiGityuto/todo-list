...
3. デザインの方向性を一緒に決める
   ...
   - スタイルガイドをつくる
     ...
     - 特に、色とフォントサイズは後から統一するの面倒なので最初に厳密に決めた方が良い。
       - 色
         - メインカラー（ブランドカラー/プライマリーカラー）、ベースカラー（背景色、フォントカラー）、アクセントカラー（セカンダリーカラー）決める
         - 色を決めるときはコントラストに注意する（WCAGの基準を参考にする）
         - borderやhoverの色もルールを決める。
           - hoverは基本的には元の色の明度を上げ下げする。
           - 明度を上げる場合は、opacityで調整するのが簡単。（もしくは、primary-hoverみたいな色定義をつくるのもあり）
           - 明度を下げる場合は、xxx-hoverみたいな色定義を作るのが良いと思う（card-hoverとか）
         - ラベルとかは区別するために色々な色を使っても良いが、それ以外は、基本的にはあらかじめ決めた色だけを使う。
         - 特にグレー系の色はバリエーションが多くなりがちなので、用途に応じて決めて、それ以外は使わないようにする
           - 非アクティブな要素の色、プレースホルダーの色、シェブロン系のアイコンの色など、用途ごとに色を決める。個人的には最初は全部同じ色にしておいて、変えたくなったら色を分けるのが良いと思う。
       - フォントサイズ
         - 見出しは24px、本文は16px、注釈は12pxなど。（15pxとか中途半端な数字は避けて、TailwindCSSのデフォルトに従うのがベター）
     - ボタンのスタイルもある程度決める（以下は一例）
       - 一番重要なボタン: メインカラーで塗りつぶし。フォントサイズは16px
       - 二番目に重要なボタン: アクセントカラーで塗りつぶし。フォントサイズは16px
       - それほど重要でないボタン: アクセントカラーで枠線のみ。フォントサイズは16px
       - 危険なボタン: 赤色で塗りつぶし。フォントサイズは16px
       - ボタンのheightはvariantで決める（large, small）。heightは直接指定せずにpaddingで調整。
       - ボタンのwidthはautoかfullか。widthは上書きできるようにする。paddingは両端に均等に入れる。
       - ボタンのテキストは16pxか12pxか
       - border-radiusは50%の丸ボタン
       - shadowはなし
     - 作成方法
