...
Phase 2-2 の各ページ実装時に、どこに何を置くか迷わないための指針。
モデル定義方法（type vs class）が未決定のため、WIP として管理する。
...
- モデル定義方法 → [005-task-model-definition.md](./005-task-model-definition.md)
- `types/` vs `models/` のディレクトリ名
- `use-local-storage.ts` を汎用フックとして切り出すかどうか

## ステータス

WIP → モデル定義方法の決定後に確定する
