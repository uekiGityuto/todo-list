...
### #14 削除が即実行で誤操作リスクがある → 完了

- **状態**: 完了
- **画面**: タスク一覧（タスクアクションダイアログ）
- **問題**: 削除ボタンを押すと確認なしで即削除されるため、誤タップで削除してしまうリスクがある
- **対応**: 案Bを採用。タスクアクションをボトムシートからアコーディオン展開（カード直下に展開）に変更し、削除タップ時に確認ダイアログを表示する方式に変更
  - タスクカード/次やるタスクカードのタップでアクション一覧をアコーディオン展開
  - 削除確認ダイアログを新規追加（Dialog - Delete Confirm）
  - 既存のDialog - Task Action / Dialog - Next Task Actionを削除
  - TaskCard/NextTaskCardコンポーネントのシェブロンをchevron-downに変更
  - 実装時の詳細は [`docs/specs/implementation-notes.md`](../specs/implementation-notes.md) に記載
